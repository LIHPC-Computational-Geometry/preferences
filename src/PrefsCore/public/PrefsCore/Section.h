 ...
 * Une section est composée d'éléments, pouvant être des
 * sous-sections ou des valeurs nommees.
...
class Section : public Element
{
	 ...
	 */
	Section (const IN_UTIL UTF8String& name, 
	 ...
	 *				false dans le cas contraire. Descendance prise en compte.
	 */
	...
	/**
	 * @return			true si l'instance a un élément du nom tranmsis
	 *					en argument, ou false dans le cas contraire.
	 */
	virtual bool hasElement (const IN_UTIL UTF8String& name) const;

	/**
	 * @return			L'élément dont le nom est transmis en argument.
	 * @exception		Une exception est levée si l'instance n'a pas
	 *					l'élément demandée.
	 */
	virtual Element& getElement (const IN_UTIL UTF8String& name) const;

	/**
	 * @return			true si l'instance a une sous-section du nom tranmsis
	 *					en argument, ou false dans le cas contraire.
	 */
	 ...
	 * La section ajoutée est adoptée, c'est à dire que cette instance
	 * se chargera de sa destruction.
	 * @exception		Une exception est levée si cette instance a 
	 *					déjà une sous-section de ce nom.
	 */
	virtual void addSection (Section* section);

	/**
	 * Supprime la sous-section reçue en argument.
	 * @exception		Une exception est levée si l'instance n'a pas de
	 *					sous-section à supprimer.
	 */
	...
	/**
	 * @return			true si l'instance a une valeur nommée du nom 
	 *					transmis en argument, ou false dans le cas contraire.
	 */
	virtual bool hasNamedValue (const IN_UTIL UTF8String& name) const;

	/**
	 * Ajoute la valeur nommée reçue en argument à la liste des 
	 * valeurs nommées.
	 * La valeur nommée ajoutée est adoptée, c'est à dire que cette
	 * instance se chargera de sa destruction.
	 * @exception		Une exception est levée si cette instance a 
	 *					déjà une valeur nommée de ce nom.
	 */
	 ...
	 */
	virtual void renameElement (const IN_UTIL UTF8String& name, 
	                            const IN_UTIL UTF8String& newName);

	/**
	 * @param		Nouveau nom de la section.
	 * @exception	Une exception est levée si cette section a un parent
	 *				(utiliser alors renameElement sur le parent).
	 */
	virtual void rename (const IN_UTIL UTF8String& name);

	/**
	 * Affichage dans le flux reçu en argument de la section et de sa
	 * descendance.
	 * @param		Flux utilisé
